...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header ">Receipt <a href="{{ route('receipt.create') }}"id="btn_add" name="btn_add" class="btn btn-primary pull-right">Add New</a></h1>
                ...
                <div class="panel-body">
                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        @foreach ($receipt as $tax)
                            <tr class="odd gradeX">
                                <td>{{$tax->receipt_no}} </td>
                                <td>{{$tax->total_amount}}</td>
                                <td>{{date('Y-m-d',$tax->date)}}</td>

                                <td class="center">
                                    <form action="{{ route('receipt.destroy',$tax->id) }}" method="POST">
