...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header ">Sales Tax <a href="{{ route('sales-tax.create') }}"id="btn_add" name="btn_add" class="btn btn-primary pull-right">Add New</a></h1>
                ...
                <div class="panel-body">
                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        @foreach ($salesTaxes as $tax)
                            <tr class="odd gradeX">
                                <td>{{$tax->client->first_name}} &nbsp; {{$tax->client->last_name}}</td>
                                <td>{{$tax->client->type}}</td>
                                <td>{{date('Y-m-d',$tax->date)}}</td>
                                <td class="center">{{$tax->value}}</td>
                                <td class="center">
                                    <form action="{{ route('sales-tax.destroy',$tax->id) }}" method="POST">
